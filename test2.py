...
if __name__ == "__main__":
    ...
    if client_id:
        ...
        interpretation_df = interpretation_df.sort_values('SHAP Value', ascending=False)

        # Afficher les caractéristiques influentes positivement
        st.write("Caractéristiques influentes positivement:")
        selected_positive_features = st.multiselect(
            "Sélectionnez les caractéristiques influant positivement",
            interpretation_df[interpretation_df['SHAP Value'] > 0]['Feature'].tolist()
        )

        # Afficher les caractéristiques influentes négativement
        st.write("Caractéristiques influentes négativement:")
        selected_negative_features = st.multiselect(
            "Sélectionnez les caractéristiques influant négativement",
            interpretation_df[interpretation_df['SHAP Value'] < 0]['Feature'].tolist()
        )

        # Afficher les histogrammes pour les caractéristiques sélectionnées positivement
        if selected_positive_features:
            plot_distribution(selected_positive_features, client_id)

        # Nouveau bouton pour afficher les caractéristiques influant négativement
        if st.button('Afficher les caractéristiques influant négativement'):
            plot_distribution(selected_negative_features, client_id)
